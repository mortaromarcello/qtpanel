...
void DockItem::close()
{
	X11Support::closeWindow(m_client->handle());
}
...
{
	startAnimation();
	//PanelWindow::Anchor anchor = PanelApplication::instance()->getVerticalAnchor();
	//m_label->resize(m_client->pixmap().size());
	//m_label->setPixmap(m_client->pixmap());
	//QPoint dock_pos = (m_dockApplet->getPosScreen()) + QPoint(0, (anchor == PanelWindow::Top) ? m_size.height() : 0);
	//QPoint label_pos(m_position.x() + dock_pos.x() + m_size.width()/2 - m_label->size().width()/2, m_position.y() + dock_pos.y() - ((anchor == PanelWindow::Top)? 0 : m_label->size().height()));
	//MyDBG << "m_position:"<< m_position << ", dockapplet pos:"<< dock_pos << ", label size:" << m_label->size() << ", handle:" << m_client->handle() << ", pixmap is null:" << m_label->pixmap()->isNull() << "isHide:" << m_label->isHidden() << ", label pos:" << label_pos;
	//m_label->move(label_pos);
	//m_label->show();
	//X11Support::activateWindow(handle);
	//MyDBG << "isShow:" << !m_label->isHidden();
	//
}
	...
	//m_pixmap(QPixmap())
{
	...
	updateIcon();
	updateUrgency();
	X11Support::activateWindow(m_handle);
	int currentDesktop = xfitMan().getActiveDesktop();
	if (currentDesktop != getDesktop())
		xfitMan().setActiveDesktop(getDesktop());
	//m_pixmap = QPixmap::grabWindow(m_handle).scaledToHeight(96);
	//if (currentDesktop != getDesktop())
	//	xfitMan().setActiveDesktop(currentDesktop);
}
...
void DockApplet::updateClientList()
{
	...
	{
		Client* client = getClientFromDockItems(windows[i]);
		if (!client)
		{
			// Skip our own windows.
			if(QWidget::find(windows[i]))
				continue;
			client = new Client(windows[i]);
			if (client->isVisible()) {
				registerDockItem(new DockItem(this, client));
#ifdef __DEBUG__
				MyDBG << "added client:" << client->name();
#endif
			}
		}
	}
	...
	for(;;)
	{
		...
		foreach(DockItem* item, m_dockItems)
		{
...
void DockApplet::windowPropertyChanged(unsigned long window, unsigned long atom)
{
	...
	if(getClientFromDockItems(window) != NULL) {
