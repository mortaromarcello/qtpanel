...
QXClient::QXClient( Window win, const XWindowAttributes &attr )
	: mId( win ),
	  mPixmap( None ),
	  mPicture( None ),
	  mAlphaPict( None ),
...
void QXClient::createWindowPicture()
{
...
void QXClient::geometryChanged( const QRect &newGeometry, int newBorderWidth )
{
...
QRect QXClient::geometry( ShapeType type ) const
{
	QRect r;
...
XserverRegion QXClient::createRegion( ShapeType type ) const
{
...
void QXClient::updateOnScreenRegion()
{
	if ( mVisibleRegion != None )
...
ulong QXClient::getWindowOpacity() const
{
	Atom actual;
	int format;
	ulong nitems, left;
...
void QXClient::createOpacityMask()
{
