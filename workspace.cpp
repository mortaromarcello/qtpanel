...
QXClient *Workspace::find( Window id ) const
{
...
void Workspace::restack( QXClient *client, Window above /* the window below client */)
{
	QMutableLinkedListIterator<QXClient *> i(mList);
...
void Workspace::initClientList()
{
	...
	for ( uint i = 0; i < nwindows; i++ ) {
		...
		mList.prepend( new QXClient( windows[i], attr ) );
	}
...
void Workspace::repaint()
{
	...
	// QXClient list for clients that are either translucent, or have a shadow
...
void Workspace::createNotifyEvent( const XCreateWindowEvent *event )
{
	...
	QXClient *client = new QXClient( event->window, attr );
...
{
	QXClient *client = find( event->window );
...
{
	QXClient *client = find( event->drawable );
...
bool Workspace::x11Event( XEvent *event )
{
	switch ( event->type ) {
		...
		case MapNotify:
			if ( QXClient *client = find( event->xmap.window ) )
				client->show();
			break;

		case UnmapNotify:
			if ( QXClient *client = find( event->xunmap.window ) )
				client->hide();
			break;
